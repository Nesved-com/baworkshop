...
export const MISSION_META: Record<SimulatorSection, MissionMeta> = {
  ...
  stakeholders: {
    ...
    quiz: {
      question: 'If the BA builds the payment portal without consulting the RBI Compliance team, what is the risk?',
      answer: 'The portal may go live without PCI-DSS certification, making every transaction a legal liability for ShopEase.',
    },
    faculty: {
      ...
      expectedAnswers: ['ShopEase leadership — revenue recovery', 'Payment Gateway / RBI compliance team', 'A simple, fast checkout that doesn\'t confuse her'],
      example: 'If NPCI rules for UPI are not understood by the BA, the UPI flow may be built incorrectly and fail RBI audit before launch.',
  ...
  summary: {
    ...
    faculty: {
      ...
      example: 'Without the BA: ShopEase adds a UPI button → fails PCI-DSS audit → goes live without compliance → RBI penalty → worse than before. With the BA: every step is planned, validated, and measurable.',
