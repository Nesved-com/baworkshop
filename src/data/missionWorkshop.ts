...
export const MISSION_META: Record<SimulatorSection, MissionMeta> = {
  ...
  'payment-simulator': {
    ...
    completion: 65,
    time: '12 min',
    story: 'Rahul tries each payment method — UPI, Card, EMI, Wallet, BNPL, Net Banking. As a BA, you observe what inputs are needed, what can fail, and what the confirmation must show.',
    explanation: 'Try each payment method as a customer. Then switch to BA mode and document what you observed.',
    activity: 'Each student tests one payment method and notes: what inputs, what errors, what confirmation.',
    revealConcept: 'Every field, error message, timeout, and success screen is a requirement the BA must define.',
    keyLearning: 'A BA who experiences the product writes better requirements than one who only reads specs.',
    quiz: {
      question: 'If UPI payment fails, what should the system do?',
      answer: 'Preserve the cart, show a clear error reason, and offer retry or alternative payment method.',
    },
    faculty: {
      objective: 'Give students a hands-on feel for the payment portal they are analysing.',
      notes: 'Encourage students to click through failure paths, not just the happy path.',
      questions: ['What does the success screen need to show?', 'What happens if the OTP expires?'],
      expectedAnswers: ['Order ID', 'Amount paid', 'Receipt', 'Retry option', 'Cart preserved'],
      example: 'A card OTP timeout should not charge the customer but must clearly say why payment was not completed.',
      takeaway: 'Observation generates better requirements than assumptions.',
    },
  },
  brd: {
    id: 'brd',
