...
export const PRIYA_STORY =
  'Rahul, a working professional in Mumbai, wants to buy a ₹12,000 kitchen appliance on ShopEase. At checkout she finds only Cash on Delivery. She doesn\'t keep large cash at home, misses the delivery agent twice, and finally cancels the order. ShopEase loses the sale — and Rahul buys from Amazon instead.'
...
export const MISSION_META: Record<SimulatorSection, MissionMeta> = {
  dashboard: {
    ...
    faculty: {
      ...
      example: 'Rahul cancels a ₹12,000 COD order because she isn\'t home during delivery. ShopEase loses that sale permanently.',
  ...
  'business-problem': {
    ...
    faculty: {
      ...
      example: 'Rahul doesn\'t just leave — she buys from Amazon. ShopEase doesn\'t just lose ₹12,000. It loses Rahul as a lifetime customer.',
  ...
  stakeholders: {
    ...
    completion: 20,
    time: '5 min',
    story: 'Rahul is one customer. Behind her abandoned order are 7 teams — each with different goals and different definitions of success for the same project.',
    ...
    activity: 'Quick role play: one student speaks as Rahul (customer), one as Finance (wants T+1 settlement), one as Dev (wants clear specs). What does each need from the BA?',
    ...
    faculty: {
      ...
      questions: ['Who has the most to gain if this project succeeds?', 'Who could block this project if not consulted early?', 'What does Rahul need that only the UX team can provide?'],
  ...
  'as-is-process': {
    ...
    completion: 30,
    time: '5 min',
    story: 'Rahul\'s current ShopEase journey: Browse → Add to Cart → Reach Checkout → See only COD → Decide to think about it → Forget → Competitor wins.',
    ...
    activity: 'Students narrate Rahul\'s journey step by step — then identify the exact step where ShopEase loses her.',
    ...
    quiz: {
      question: 'At which step does ShopEase currently lose Rahul as a customer?',
      answer: 'At the Payment Selection step — where COD is the only option. Rahul needs digital payment and ShopEase cannot offer it.',
    },
    faculty: {
      ...
      questions: ['How many steps in the COD process involve a human being?', 'What happens when the delivery agent doesn\'t carry change?', 'How long does it take ShopEase to know if Rahul paid or returned?'],
      expectedAnswers: ['5+ steps involve humans', 'The sale fails at doorstep', '3-5 days for manual reconciliation'],
      example: 'In a digital payment, ShopEase knows within 5 seconds whether Rahul paid. In COD, they know after 3-5 days and a human-driven return process.',
  ...
  'pain-points': {
    ...
    completion: 40,
    time: '5 min',
    story: 'Rahul doesn\'t complain to ShopEase. She just leaves. The business doesn\'t know why 86,000 carts are abandoned every month — until the BA investigates.',
  ...
  'to-be-process': {
    ...
    completion: 60,
    time: '6 min',
    story: 'The new ShopEase: Rahul browses, adds to cart, reaches checkout, sees 6 payment options — picks UPI, pays in 8 seconds, gets a digital receipt, and tells her friends. ShopEase keeps the sale and gains a loyal customer.',
    ...
    quiz: {
      question: 'In the To-Be process, what happens if Rahul\'s UPI payment fails?',
    ...
    faculty: {
      ...
      notes: 'Connect every To-Be improvement directly to a pain point from Mission 4. Students should see that no improvement is invented — each one directly addresses something that caused Rahul to leave.',
  ...
  'payment-simulator': {
    ...
    completion: 65,
    time: '10 min',
    story: 'The portal is built. Rahul visits ShopEase and now sees 6 payment options. As a BA, you must experience every option — not as a developer, but as Rahul. What does she need to see? What happens when it fails? What does the confirmation look like?',
    ...
    quiz: {
      question: 'If Rahul enters a wrong UPI VPA, what should the system do?',
    ...
    faculty: {
      ...
      questions: ['What inputs does the UPI screen need from Rahul?', 'What should happen if the OTP expires before Rahul enters it?', 'What must the success screen show for Rahul to feel confident?'],
  ...
  brd: {
    ...
    completion: 70,
    time: '7 min',
    story: 'The development team joins tomorrow. They have never met Rahul. They don\'t know ShopEase is losing ₹64 Crore. They only have what the BA writes. The BRD is the only bridge between business understanding and technical execution.',
  ...
  'user-stories': {
    ...
    completion: 78,
    time: '6 min',
    story: 'The BRD tells the business what will be built. User Stories tell the development team what to build next — from Rahul\'s point of view. The same requirement, written differently for a different audience.',
    ...
    activity: 'Students write one story for Rahul (UPI payment) and one for the Finance team (T+1 settlement report). Then compare — same system, completely different perspectives.',
    ...
    faculty: {
      ...
      questions: ['Who is the actor for the story "receive T+1 settlement report"?', 'What is the business value of Rahul receiving an instant payment receipt?', 'Can a user story exist without a business benefit?'],
      ...
      example: '"As Rahul, I want to pay via UPI, so that I can complete my purchase in under 10 seconds without keeping cash ready." — This story connects customer convenience to ShopEase\'s cart abandonment reduction goal.',
  ...
  'acceptance-criteria': {
    ...
    completion: 86,
    time: '5 min',
    story: 'The developer builds Rahul\'s UPI payment feature. How does the BA know it was built correctly? Not by looking at the code — by checking the Acceptance Criteria written before a single line was coded.',
    ...
    activity: 'Show two criteria: (A) "UPI should work" vs (B) "Given Rahul enters a valid UPI VPA and clicks Pay, when the bank approves, then the order is created and a success screen appears within 5 seconds." Ask which one the QA team can test.',
    ...
    faculty: {
      ...
      example: 'Given Rahul\'s UPI payment fails due to bank decline, when the failure screen appears, then her cart must be preserved, the failure reason must be shown, and a retry button must be visible within 2 seconds.',
  ...
  uat: {
    ...
    completion: 94,
    time: '5 min',
    story: 'The payment portal is built. Before Rahul uses it in production, business users must test every scenario the BA defined — not just the happy path. UAT is the BA\'s final quality gate.',
    ...
    quiz: {
      question: 'Rahul pays via UPI, the app closes mid-transaction, and the bank deducts money but no order is created. Whose responsibility is this scenario?',
  ...
  quiz: {
    ...
    completion: 97,
    time: '4 min',
    story: 'Rahul\'s journey — from abandoned COD cart to confident UPI payment — is now your case study. Can you connect every BA artifact back to a moment in her story?',
    ...
    keyLearning: 'BA concepts are only useful when applied to real problems. Rahul\'s story was not decoration — it was the thread connecting every mission.',
    quiz: {
  ...
  summary: {
    ...
    completion: 100,
    time: '2 min',
    story: 'Rahul returns to ShopEase. She sees 6 payment options. She pays via UPI in 8 seconds. She gets an instant receipt. ShopEase gets a confirmed sale, T+1 settlement, and a loyal customer. The BA made this possible — not by coding, but by thinking clearly.',
    ...
    faculty: {
      ...
      notes: 'End on Rahul. She started as an abandoned cart. She ends as a successful, satisfied customer. The BA created the conditions for that outcome — not by building anything, but by thinking clearly and documenting precisely.',
